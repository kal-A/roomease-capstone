...
export function ConfirmationPage({
  ...
  return (
    <div className="min-h-[100dvh] flex flex-col items-center justify-center max-w-lg mx-auto px-4 py-4 gap-3">
      <div className="text-center flex-shrink-0">
        <motion.div
          className="mx-auto flex h-10 w-10 items-center justify-center rounded-full bg-[var(--primary)]/15 ring-2 ring-[var(--primary)]/30"
          initial={{ scale: 0.8, opacity: 0 }}
          animate={{ scale: 1, opacity: 1 }}
          transition={{ duration: 0.25, ease: "easeOut" }}
        >
          <svg className="h-5 w-5 text-[var(--primary)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round">
            <motion.path d="M5 13l4 4L19 7" initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ delay: 0.1, duration: 0.35, ease: "easeOut" }} />
          </svg>
        </motion.div>
        <h1 className="mt-2 text-base font-bold text-[var(--text)]">Booking Confirmed</h1>
        <p className="mt-0.5 text-xs text-[var(--textSecondary)]">Your confirmation ticket is below.</p>
      </div>
      <div className="flex-1 min-h-0 w-full [&_.rounded-xl]:py-3 [&_.rounded-xl]:px-3 [&_.rounded-xl]:text-sm">
      ...
      <div className="flex flex-wrap justify-center gap-2 flex-shrink-0">
        <Link href="/bookings" className="rounded-xl bg-[var(--primary)] px-5 py-2 text-sm font-semibold text-black shadow-sm hover:bg-[var(--primaryHover)] focus:outline-none focus:ring-2 focus:ring-[var(--focusRing)]">
          View My Bookings
        </Link>
