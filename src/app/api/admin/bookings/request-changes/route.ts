...
export async function POST(req: Request) {
  ...
  // Fetch booking so we can validate it's real and so clients can't send arbitrary ids.
  ...
  // Determine whether the room requires approval.
  ...
  // Mark as changes requested for approval-policy rooms; otherwise keep it confirmed.
  const resolvedStatus = roomRequiresApproval ? "changes_requested" : "confirmed";

  const payload: Record<string, unknown> = {
    status: resolvedStatus,
    review_state: roomRequiresApproval ? "changes_requested" : null,
    requested_changes_at: roomRequiresApproval ? nowIso : null,
    admin_note: body?.adminNote && String(body.adminNote).trim() ? String(body.adminNote).trim() : null,
  };
  ...
  if (updateErr) {
    console.error("SUPABASE REQUEST CHANGES UPDATE ERROR", { bookingId, updateErr });
